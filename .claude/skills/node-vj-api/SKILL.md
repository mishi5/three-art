...
1. **一式を起動**: `bun run bridge:up [devPort] [relayPort]`（既定 3000/8787。中継＋dev サーバをバックグラウンド起動し PID を記録、macOS ではブラウザでページまで自動で開く（`--no-open` で抑止）。**対象ポートが使用中なら既存サーバに触れず中止**する）。停止は `bun run bridge:down`（記録した自前プロセスのみ停止）。中継単体なら `bun run relay`（127.0.0.1:8787。port は第 1 引数 or 環境変数 `VJ_RELAY_PORT`）
