...
6. コミット・プッシュ → PR 作成（**この時点ではマージしない**）
   - **PR 本文に `Closes #xx` / `Fixes #xx` を書かないこと。** マージ時に Issue が自動クローズされてしまうため。
7. **マージ前コンフリクトチェック**（ユーザ確認の前に行う）
   - 最新の main を取得し（`git fetch origin main`）、現ブランチを main にマージした場合にコンフリクトが発生するか確認する。
   - コンフリクトが発生する想定であれば、**ユーザ確認の前に** main へ rebase（または merge）してコンフリクトを解消し、全テストを再実行して全件パスを確認する。解消結果を push する。
   - コンフリクトしない、または解消・テストOKになってから次へ進む。
8. ユーザに動作確認を促す（push 済みブランチ / PR で確認してもらう）
   - **動作確認を依頼する際は、実際に作業した worktree への `cd` と実行コマンドを `&&` で繋いだ、そのままコピペできる 1 行を必ず提示すること。** パスはプレースホルダではなく実パスを書く。
   - 動作確認に複数手順がある場合は、それぞれ `cd <実パス> && <コマンド>` の形で提示する。
9. ユーザが確認OKを出した後に、はじめて PR をマージする
10. マージ後、Issue に対応内容を日本語でコメントしてクローズする
11. worktree・ブランチを削除、main を pull
