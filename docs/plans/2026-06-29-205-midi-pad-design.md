...
filled パッドのクリック操作（ノード／全画面オーバーレイ共通）:
- **通常クリック** = 発音
- **Cmd/Ctrl+クリック** = そのパッドの**発音中の音だけ個別停止**（`stopPad(index)`）
- **Shift+クリック** = 再割当（ファイルダイアログ）。**再割当を始めた時点でそのパッドの音を停止**してから差し替える
- **Alt+クリック** = **割当解除**（`clearPad`＝音を止め buffer/ラベルを空に・`padAssets[slot]` を空文字に）
- 空パッドのクリック = 割当

ランタイム: `active` を `Map<AudioBufferSourceNode, padIndex>` に変更し、`stopPad(i)`（個別停止）/`clearPad(i)`（解除）を追加。`stopAll()`（■ 全停止ボタン）は維持。アセットのパッドへのドロップ／ファイル再割当でも、差し替え前に `stopPad` で古い音を止める。

## 追補2: パッド操作を右クリックメニューに統一（修飾キー廃止）

修飾キー（Shift/Alt/Cmd+クリック）は覚えにくく、拡大オーバーレイの `<button>` では Shift+クリックが
別ウィンドウを開く不具合の温床にもなっていたため、操作を整理した:

- **左クリック = 発音のみ**（空パッドは何もしない）。
- **右クリック = パッド操作メニュー**:
  - 空パッド: 「音声を割り当て」
  - 音入り: 「再生」「このパッドを停止（個別）」「音声を再割り当て」「割り当てを解除」
- ノードの ■（全停止）/ ⛶（拡大）ボタンは従来どおり。

実装:
- ノード: `NodeEditor.openContextMenu` でパッド上の右クリックを検出し `showPadMenu`（既存メニュー機構を流用）。
- オーバーレイ: パッド `<button>` を `type="button"` にし、左クリック=発音 / `contextmenu` で DOM メニュー
  `showPadCtxMenu`（cursor 位置・画面端回避・外側クリック/Esc で閉じる）。
- 再割当・割当・解除・個別停止はいずれも既存コールバック（onAssignPad/onUnassignPad/onStopPadVoice、
  overlay は deps.assign/unassign/stopVoice）をメニューから呼ぶ。runtime の stopPad/clearPad は維持。
