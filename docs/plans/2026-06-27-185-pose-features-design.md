...
レビュー指示により以下を変更:
- **`jump`（trigger）出力を削除**（不要との判断）。関連する `jumpThreshold` param・`jumpStep` ロジック・jump 武装状態も撤去。
- **`motion` は CameraInput にも存在していたため、CameraInput 側の `motion` 出力を削除**し、動き量は PoseFeatures に集約。
- 結果、PoseFeatures の出力は **MVP 3 本（handHeightL / handHeightR / motion、すべて number）**。

以下、本文の「ジャンプ」「jump」「4 本」記述は上記変更後（jump 無し・3 本）に読み替え。

## 目的

MediaPipe Pose を「描画素材」だけでなく **演出の制御信号源** に昇格させる。`pose`（`PoseFrame`）を入力に取り、
手の高さ・全身の動き量を number として出力する `PoseFeatures` ノードを追加し、任意の param を
...
- `center` は可視度重み付き重心（y 上が +）。本ノードでは未使用（jump 削除により）。

### motion について
`PoseFrame` には動き量が含まれない。本ノードは `pose` のみを入力に取るため、**`JointAnchors.getMotion()` のアルゴリズム
（可視度重み付きの毎フレーム関節変位・指数平滑 0.85/0.15）を本ノード内で前フレーム関節との差分から再現**する。
動き量はこの PoseFeatures に集約し、CameraInput からは `motion` 出力を削除した（`JointAnchors.getMotion()` 自体は core に存置）。
...
- pose 未接続（`input("pose")` が undefined）→ 全出力 0。
- 肩 or 対象手首が `visibility < 0.4` → その手の高さは **literal 0**（Remap を通さず spurious 信号を出さない）。
- pose が一旦切れたら前フレーム関節を無効化（`hasPrev=false`）。復帰時に motion がスパイクしない。
...
- category: `input`（CameraInput と同カテゴリ）。inputs: `pose`。outputs: 上記 3 本。
- フレーム間状態 `PoseFeaturesRuntime`（前フレーム関節・手の高さ/motion の EMA 値）を `createState`。
- registry に登録（input セクション）。

## テスト方針

- 純粋ロジック: 肩幅正規化が体格非依存・raiseSpan/不可視 0・motion 平滑をユニットテスト。
- ノード統合: `evaluate()` 経由で pose 注入 → 出力検証（math-nodes.test.ts 流儀）。未接続 0・Remap 反映・motion・復帰時スパイクなし。
