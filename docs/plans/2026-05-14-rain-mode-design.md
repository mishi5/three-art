...
- **位相累積バグ修正**: 位置を `v * 経過時間` で算出していたため、音量で v が
  毎フレーム変動すると移動量が `Δv * t` (t=巨大) ジャンプし瞬間移動していた。
  bin ごとの変位を CPU で `+= v*dt` 累積し H でラップ、シェーダは変位を引くだけに
  変更（`stepDisplacement`）。dt はクランプ。
- **log マッピング本実装**: 画面X位置 (一様) と参照 bin (マッピング) を分離
  （`mapBinIndex`）。既定を log に変更し、無音の高域が画面の大半を占める問題を解消。
- 既定値を控えめに調整 (baseSpeed 0.3→0.15, ampGain 4→2, length 0.04→0.06)。
