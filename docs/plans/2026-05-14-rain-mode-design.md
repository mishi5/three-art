...
## 動作確認フィードバック反映 (2026-05-16)

### 第1次

- **位相累積バグ修正**: 位置を `v * 経過時間` で算出していたため瞬間移動していた。
- **log マッピング本実装**: 画面X位置 (一様) と参照 bin (マッピング) を分離。

### 第2次 — 粒子単位モデルへ再設計

列単位 (bin ごとの共有変位) では列全体が一斉に伸縮し不自然、無音時は等速・
等間隔・決定的位相で「静的格子(モアレ)」に見えた。ユーザ要望により以下に変更:

- **粒子ごとに速度を保持**。生成 (上端リスポーン) の瞬間にその bin の振幅で
  速度を確定し、落下中は維持 (`pickSpawnSpeed` / `advanceParticleY`)。音の履歴が
  空間パターンとして残る本物の雨の挙動になる。
- 粒子ごとに X ジッタ・Y 初期位相・速度±15%・長さ±20% の個体差。無音時も
  自然なドリズルに見え、高音域帯の違和感も解消。
- GPU の DataTexture を撤去し位置を CPU で毎フレーム計算 (GLSL の罠減・堅牢化)。
  attribute は position と aSpeed (明度用) のみ。
- 既定値: baseSpeed 0.12 / ampGain 1.0 / length 0.05。
- スライダ範囲を微調整向けに縮小: baseSpeed 0–0.8(step .005) /
  ampGain 0–4(step .02) / length 0–0.2(step .002)。
- areaWidth/Height/mapping/count 変更で粒子バッファを再構築。

純粋関数 `expectedRainSpeed` / `mapBinIndex` / `pickSpawnSpeed` /
`advanceParticleY` を単体テスト。
