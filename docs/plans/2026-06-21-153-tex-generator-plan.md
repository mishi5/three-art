...
## 設計
- 新ノード `TextureGeneratorNode`（category: `visual`、入力なし、`texture` 出力）。
  - feature-reference では「入力・素材系のジェネレーター」。外部入力でない生成元なので visual に置く（出力が texture＝画面に出す素材で RainVisual 等と同じ括り。PointShape は points を出す上流ソースなので generator に残す）。
...
- `textureGenModeInt(mode)` 純関数（solid=0/linear=1/radial=2, 未知=1）。

## テスト
- `texture-generator.test.ts`: 定義（category/入力なし/texture 出力）・mode enum・RGB number param・angle・
  textureGenModeInt・registry 登録・headless evaluate no-op。
- GLSL 描画は Playwright スモークでコンパイル＆グラデ描画を確認。

## 成果物
- `nodes/TextureGeneratorNode.ts`（新規）・registry 登録・テスト。全733件パス。
