...
タップ/onset の時刻列 → `recentIntervals` → `estimateBpm`（fold ＋ 中央値）のパイプライン。
...
3. `estimateBpm(intervals, minCount=3)`: fold 後の有効間隔が 3 未満なら null。中央値 → 60/median。
   中央値ベースなので単発の外れ値に頑健。
...
これで「8分打ち 0.25s → 120」「2拍毎 1.0s → 120」が成立する。トレードオフとして推定 BPM は
この帯に丸まる（例: 174BPM のタップは 87 と推定される）が、倍/半の関係なので拍位相の同期用途では
実害がなく、帯外のテンポが必要なら `bpm` param の手動設定（30〜300）で全域を指定できる。
