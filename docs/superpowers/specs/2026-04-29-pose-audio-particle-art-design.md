...
- **開発サーバ**：`bun ./pose-particles.html` — Bun 1.2+ のフルスタックサーバが HTML をエントリにして TypeScript / GLSL のバンドルと HMR を担う（複数作品を立ち上げる場合は `bun ./*.html`）
- **本番ビルド**：`bun build ./pose-particles.html --outdir dist`
- **パッケージ管理**：`bun install`
...
このリポジトリは複数の Three.js アート作品を集約することを想定し、`src/` 配下に**作品ごとのディレクトリ**を切る。今回の作品の作業ディレクトリ名は **`pose-particles`** とする（後でリネーム可）。

```
three-art/
├── index.html                          # ランディング（作品リスト）
├── pose-particles.html                 # この作品の HTML エントリ
├── package.json
├── tsconfig.json
├── .gitignore
├── public/
...
│   └── superpowers/
│       └── specs/
│           └── 2026-04-29-pose-audio-particle-art-design.md
└── src/
    └── pose-particles/                 # この作品の全コード
        ├── main.ts
        ├── App.ts
        ├── types.ts
        ├── audio/
...
**新作品を足すときの規約**：
- `src/<作品名>/` にコードを置く
- `<作品名>.html` をリポジトリ直下に置き、`<script type="module" src="/src/<作品名>/main.ts">` を読む
- 必要なら `public/<作品名>/` にアセットを置く
- 共有ユーティリティができてきたら `src/shared/` を作る（今は不要）
