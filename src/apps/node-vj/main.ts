...
// ===== TEMP #167 デバッグ計測（原因切り分け後に削除する）=====
// 実機トラックパッドで pointer/key イベントの実値を記録する。
// 再現後にブラウザのコンソールで copy(window.__evlog) するか、[#167dbg] 行を貼ってください。
{
  const ed = editor as unknown as { drag: { kind: string } | null; spaceDown: boolean };
  const evlog: Array<Record<string, unknown>> = [];
  (window as unknown as { __evlog: unknown }).__evlog = evlog;
  const t0 = performance.now();
  const rec = (tag: string, e: PointerEvent | KeyboardEvent): void => {
    const r: Record<string, unknown> = {
      ms: Math.round(performance.now() - t0),
      tag,
      drag: ed.drag ? ed.drag.kind : null,
      space: ed.spaceDown,
    };
    if (e instanceof PointerEvent) {
      r.button = e.button; r.buttons = e.buttons; r.ptype = e.pointerType;
    } else {
      r.key = e.key; r.code = e.code; r.repeat = e.repeat;
    }
    evlog.push(r);
    if (evlog.length > 400) evlog.shift();
    console.log("[#167dbg]", JSON.stringify(r));
  };
  for (const t of ["keydown", "keyup"] as const) {
    window.addEventListener(t, (e) => rec(t, e), true);
  }
  for (const t of ["pointerdown", "pointerup", "pointercancel"] as const) {
    window.addEventListener(t, (e) => rec(t, e), true);
  }
  let lastMove = 0;
  window.addEventListener("pointermove", (e) => {
    const now = performance.now();
    // パン中（drag あり）は全部、それ以外は ~12fps に間引いて記録。
    if (ed.drag || now - lastMove > 80) { lastMove = now; rec("pointermove", e); }
  }, true);
  console.log("[#167dbg] instrumentation ON");
}
// ===== /TEMP #167 =====
