...
export const RgbShiftNode: NodeTypeDef = {
  type: "RgbShift",
  category: "effect",
  description: "node.RgbShift.desc",
  isSink: true,
  inputs: [
    { id: "in", label: "in", type: "texture", description: "node.RgbShift.port.in" },
    { id: "trigger", label: "trig", type: "trigger", description: "node.RgbShift.port.trigger" },
  ],
  outputs: [{ id: "texture", label: "tex", type: "texture", description: "node.RgbShift.port.texture" }],
  params: [
    EFFECT_ENABLED_PARAM,
    { id: "amount", label: "amount", kind: "number", default: 0.003, min: 0, max: 0.05, step: 0.0005, description: "node.RgbShift.param.amount" },
    { id: "angle", label: "angle", kind: "number", default: 0, min: 0, max: 1, step: 0.01, description: "node.RgbShift.param.angle" },
    { id: "triggerAmount", label: "triggerAmount", kind: "number", default: 0.02, min: 0, max: 0.1, step: 0.001, description: "node.RgbShift.param.triggerAmount" },
    { id: "decay", label: "decay", kind: "number", default: 0.15, min: 0.01, max: 2, step: 0.01, description: "node.RgbShift.param.decay" },
  ],
